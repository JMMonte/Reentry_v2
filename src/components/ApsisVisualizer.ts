...
export class ApsisVisualizer {
    ...
    private periapsisMaterial!: THREE.MeshBasicMaterial;
    private apoapsisMaterial!: THREE.MeshBasicMaterial;
    private periapsisMesh!: THREE.Mesh;
    private apoapsisMesh!: THREE.Mesh;
    public visible: boolean;

    constructor(scene: THREE.Scene, color: number | string) {
        this.scene = scene;
        this.color = color;
        this.visible = false;
    ...
    private initializeApsides(): void {
        ...
        this.periapsisMesh.onBeforeRender = (
        ...
        this.apoapsisMesh.onBeforeRender = (
            renderer: THREE.WebGLRenderer,
            scene: THREE.Scene,
            camera: THREE.Camera
        ): void => {
    ...
    public update(position: THREE.Vector3, velocity: THREE.Vector3): ApsisAltitudes | null {
        const mu = Constants.earthGravitationalParameter;
        const orbitalElements = PhysicsUtils.calculateOrbitalElements(position, velocity, mu) as OrbitalElements;
