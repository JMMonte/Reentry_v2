...
class GUIManager {
    ...
    addBiImpulseControls(parentFolder) {
        ...
        biImpulseFolder.add({
            createManeuver: () => this.createBiImpulseManeuver(biImpulseData)
    ...
    createBiImpulseManeuver(biImpulseData) {
        ...
        this.showUpdateAndRemoveButtons();
    }

    showUpdateAndRemoveButtons() {
        const biImpulseFolder = this.biImpulseFolder;

        if (!this.updateManeuverController) {
            this.updateManeuverController = biImpulseFolder.add({
                updateManeuver: () => this.updateBiImpulseManeuver(this.biImpulseData)
            }, 'updateManeuver').name('Update Maneuver');
        }

        if (!this.removeManeuverController) {
            this.removeManeuverController = biImpulseFolder.add({
                removeManeuver: () => this.removeBiImpulseManeuver(this.biImpulseData)
            }, 'removeManeuver').name('Remove Maneuver');
        }
    }

    updateBiImpulseManeuver(biImpulseData) {
        const selectedSatellite = this.satellites.find(sat => `${sat.id}` === biImpulseData.selectedSatellite);
        if (!selectedSatellite) {
            console.error('No satellite selected for the maneuver.');
            return;
        ...
        selectedSatellite.maneuverNodes = selectedSatellite.maneuverNodes.filter(node => node.time !== this.currentManeuverTime);

        this.createBiImpulseManeuver(biImpulseData);
    }

    removeBiImpulseManeuver(biImpulseData) {
        const selectedSatellite = this.satellites.find(sat => `${sat.id}` === biImpulseData.selectedSatellite);
        if (!selectedSatellite) {
            console.error('No satellite selected for the maneuver.');
            return;
        ...
        this.hideUpdateAndRemoveButtons();
    }

    hideUpdateAndRemoveButtons() {
        if (this.updateManeuverController) {
            this.biImpulseFolder.remove(this.updateManeuverController);
            this.updateManeuverController = null;
        }

        if (this.removeManeuverController) {
            this.biImpulseFolder.remove(this.removeManeuverController);
            this.removeManeuverController = null;
        }
    }
