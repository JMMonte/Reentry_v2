...
import { Satellite, SatelliteMap, DebugWindow, DisplaySetting } from './types';
...
function App() {
    ...
    const [isSatelliteListVisible, setIsSatelliteListVisible] = useState < boolean > (false);
    const [debugWindows, setDebugWindows] = useState < DebugWindow[] > ([]);
    const [satellites, setSatellites] = useState < SatelliteMap > ({});
    ...
    const [app3dInstance, setApp3dInstance] = useState < App3D | null > (null);
    const [isDisplayOptionsOpen, setIsDisplayOptionsOpen] = useState < boolean > (false);
    const [isSatelliteModalOpen, setIsSatelliteModalOpen] = useState < boolean > (false);
    const app3dRef = useRef < App3D | null > (null);
    ...
    useEffect(() => {
        const handleSatelliteListUpdate = (event: CustomEvent<{ satellites: SatelliteMap | Satellite[] }>) => {
            ...
            if (event.detail?.satellites) {
                ...
                const validSatellites = satelliteArray
                    .filter((sat): sat is Satellite => sat && sat.id != null && sat.name)
                    .reduce < SatelliteMap > ((acc, sat) => {
                        acc[sat.id] = sat;
                        return acc;
                    }, {});
        ...
        document.addEventListener('satelliteListUpdated', handleSatelliteListUpdate as EventListener);
        return () => document.removeEventListener('satelliteListUpdated', handleSatelliteListUpdate as EventListener);
    }, []);

    // Handle body selection events
    useEffect(() => {
        const handleBodySelected = (event: CustomEvent<{ body: string }>) => {
            if (event.detail?.body) {
                // Set a flag to prevent re-dispatching the event
                (window as any).handlingBodySelectedEvent = true;
                handleBodySelect(event.detail.body);
                (window as any).handlingBodySelectedEvent = false;
            }
        };

        document.addEventListener('bodySelected', handleBodySelected as EventListener);
        return () => document.removeEventListener('bodySelected', handleBodySelected as EventListener);
    }, []);
    ...
    useEffect(() => {
        ...
        try {
            ...
            app.createDebugWindow = (satellite: Satellite) => {
            ...
            app.removeDebugWindow = (satelliteId: number) => {
            ...
            app.addEventListener('sceneReady', () => {
                ...
                if (app.displayManager) {
                    ...
                    const initialSettings: DisplaySetting = {};
    ...
    // Time update effect
    useEffect(() => {
        const handleTimeUpdate = (event: CustomEvent<{ simulatedTime: Date; timeWarp: number }>) => {
            const { simulatedTime, timeWarp } = event.detail;
            setSimulatedTime(simulatedTime);
            setTimeWarp(timeWarp);
        };

        document.addEventListener('timeUpdate', handleTimeUpdate as EventListener);
        return () => document.removeEventListener('timeUpdate', handleTimeUpdate as EventListener);
    }, []);
    ...
    const getNextTimeWarp = (current: number, increase: boolean): number => {
    ...
    const handleSimulatedTimeChange = (newTime: Date) => {
        const app = app3dRef.current;
        if (app?.timeUtils) {
    ...
    const handleBodySelect = (value: string) => {
        ...
        if (!(window as any).handlingBodySelectedEvent) {
    ...
    return (
        <ThemeProvider defaultTheme="dark" storageKey="ui-theme">
            <div className="relative h-screen w-screen overflow-hidden">
                <canvas id="three-canvas" ref={app3dRef as any} className="absolute inset-0 z-0" />
                <Navbar
